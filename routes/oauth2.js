    ...
    AuthCode.findOne({code:code,clientId:client.clientId,redirectUri:redirectUri}, (error, authCode) => {
        ...
        Token.findOne({clientId:client.clientId,userId:authCode.userId},(error,token)=>{
            ...
            if(!token){
                ...
                Token.create({token:newtoken, userId:authCode.userId, clientId:authCode.clientId,creationTime: new Date()}, (error) => {
                  if (error) return done(error);
  ...
  server.exchange(oauth2orize.exchange.password((client, username, password, scope, done) => {//this is working
    ...
    Client.findOne({'clientId':client.clientId}, (error, localClient) => {
      ...
      User.findOne({username:username}, (error, user) => {
        ...
        Token.findOne({clientId:client.clientId,userId:user._id},(error,token)=>{
            if(error) return done(error);
            if(!token){
                const newtoken = utils.getUid(256);
                Token.create({token:newtoken, userId:user._id, clientId:localClient.clientId,creationTime: new Date()}, (error) => {
                  if (error) return done(error);
                    return done(null, newtoken);
                });
            }else{
